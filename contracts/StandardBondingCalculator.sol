...
contract PlutusBondingCalculator is IBondingCalculator {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(PLUS)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(PLUS), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(PLUS)).decimals())).div(getTotalValue(_pair));
    }
}
