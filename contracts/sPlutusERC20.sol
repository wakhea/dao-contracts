...
contract sPlutus is IsPLUS, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgPLUS public gPLUS; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked PLUS", "sPLUS", 9) ERC20Permit("Staked PLUS") {
    ...
    function setgPLUS(address _gPLUS) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gPLUS) == address(0), "gPLUS:  gPLUS already set");
        require(_gPLUS != address(0), "gPLUS:  gPLUS is not a valid contract");
        gPLUS = IgPLUS(_gPLUS);
    }
    ...
    /**
        @notice increases rPLUS supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sPLUS of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sPLUS
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sPLUS: insufficient balance");
    }
    ...
    // toG converts an sPLUS balance to gPLUS terms. gPLUS is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gPLUS.balanceTo(amount);
    }

    // fromG converts a gPLUS balance to sPLUS terms. sPLUS is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gPLUS.balanceFrom(amount);
    }

    // Staking contract holds excess sPLUS
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gPLUS.balanceFrom(IERC20(address(gPLUS)).totalSupply())).add(
